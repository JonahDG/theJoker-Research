...
def main():
	...
	for star in stars:
		...
		ticid=star['TICID']
		LombScargleFile=f'/scratch/jdg577/theJoker/Data/starsData/%s_%s/%s_%s-16TermLombScargle.fits'\
...
def getPlots(period,LCData,metaRow):
	...
	print(modes)
	#Titles
	supTitle=f'Light Curves Folded @ Modes of Best 16 Term Lomb Scargle Period\n APOGEE \
	...
	for i,ax in enumerate(axes.flat):
		...
		pngFile='/scratch/jdg577/theJoker/Plots/starPlots/'\
			+str(apogeeid)+'_'+str(ticid)+'/'\
		 	+str(apogeeid)+'_'+str(ticid)+'-LC16_Fold.png'
...
def savePDFsToSinglePDF(plots):
	...
	fullPDFFIle='/scratch/jdg577/theJoker/Plots/starPlots/PlotTypePDFs/LC16_Fold.pdf'
...
def savePNGsToSinglePDF(plots):
	...
	pdf.output('/scratch/jdg577/theJoker/Plots/starPlots/PlotTypePDFs/LC16_Fold_From_PNGs.pdf','F')
	print('Done!')
